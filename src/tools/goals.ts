...
export function registerGoalTools(server: McpServer, client: CashActivityClient): void {
  server.tool(
    "list_goals",
  ...
  server.tool(
    "delete_goal",
    "Delete a savings goal. ⚠️ Irreversible — requires confirm: true",
    {
      id: z.string().uuid().describe("Goal UUID"),
  ...
  server.tool(
    "deposit_to_goal",
    "Deposit savings into a goal from a wallet",
    {
      id: z.string().uuid().describe("Goal UUID"),
  ...
  server.tool(
    "withdraw_from_goal",
    "Withdraw savings from a goal back to a wallet. ⚠️ Requires confirm: true",
    {
      id: z.string().uuid().describe("Goal UUID"),
