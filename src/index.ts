...
import { buildRegistry, type FunctionDef } from "./registry.js";
...
async function main() {
  ...
  const fnMap = new Map<string, FunctionDef>(registry.map((f) => [f.name, f]));
  ...
  server.tool(
    "search",
    "Search available Cash Activity API functions by keyword. Returns function names, descriptions, and parameter schemas. Call this first to discover what functions exist before calling `execute`.",
    {
  ...
  server.tool(
    "execute",
    "Execute a Cash Activity API function by name. Use `search` first to discover available function names and their parameter schemas. Destructive functions require `confirm: true` in params.",
    {
