...
export function registerWalletTools(server, client) {
    server.tool("list_wallets", "List all wallets for the authenticated user", {}, async () => {
        const data = await client.get("/api/wallets");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("create_wallet", "Create a new wallet", {
    ...
    server.tool("update_wallet", "Update a wallet's name or type", {
    ...
    server.tool("delete_wallet", "Delete a wallet by ID. ⚠️ Irreversible — requires confirm: true", {
        id: z.string().uuid().describe("Wallet UUID"),
