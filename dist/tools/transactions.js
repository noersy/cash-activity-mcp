...
export function registerTransactionTools(server, client) {
    server.tool("list_transactions", "List all transactions for the authenticated user", {}, async () => {
        const data = await client.get("/api/transactions");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("create_transaction", "Create a new income or expense transaction", {
    ...
    server.tool("delete_transaction", "Delete a transaction by ID. ⚠️ Irreversible — requires confirm: true", {
