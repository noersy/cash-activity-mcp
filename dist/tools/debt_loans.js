import { z } from "zod";
export function registerDebtLoanTools(server, client) {
    server.tool("list_debt_loans", "List all debts and loans", {}, async () => {
        const data = await client.get("/api/debt-loans");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("create_debt_loan", "Record a new debt or loan", {
    ...
    server.tool("repay_debt_loan", "Record a repayment for a debt or loan. ⚠️ Requires confirm: true", {
