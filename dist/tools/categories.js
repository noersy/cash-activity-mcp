...
export function registerCategoryTools(server, client) {
    server.tool("list_categories", "List all categories for the authenticated user", {}, async () => {
        const data = await client.get("/api/categories");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("create_category", "Create a new transaction category", {
    ...
    server.tool("delete_category", "Delete a category by ID. ⚠️ Irreversible — requires confirm: true", {
