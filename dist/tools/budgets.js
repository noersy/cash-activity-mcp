...
export function registerBudgetTools(server, client) {
    server.tool("list_budgets", "List all budgets with their category info", {}, async () => {
        const data = await client.get("/api/budgets");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("upsert_budget", "Create or update a budget for a category", {
    ...
    server.tool("delete_budget", "Delete a budget by ID. ⚠️ Irreversible — requires confirm: true", {
